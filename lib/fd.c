...
seekfromcur(int fdnum, off_t offset) {
    ...
    off_t pos = fd->fd_offset + offset;
    fd->fd_offset = pos < st.st_size ? pos : st.st_size;
    return 0;
}
...
seektoend(int fd)
{
    ...
    struct Fd *fdes;
    struct Stat st;
    if ((res = fd_lookup(fd, &fdes)) < 0) return res;
    if ((res = fstat(fd, &st)) < 0) return res;
    fdes->fd_offset = st.st_size;
    return 0;
}
